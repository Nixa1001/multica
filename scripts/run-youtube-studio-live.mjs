...
const env = { ...process.env, PATH: `/home/linuxbrew/.linuxbrew/bin:${process.env.PATH ?? ""}`, DATABASE_URL: dbUrl, PORT: port, FRONTEND_PORT: webPort, NEXT_PUBLIC_API_URL: `http://127.0.0.1:${port}`, PLAYWRIGHT_BASE_URL: `http://127.0.0.1:${webPort}`, MULTICA_DEV_VERIFICATION_CODE: "428731", APP_ENV: "test" };
const children = [];
const run = (command, args, extra = {}) => {
  ...
  const child = spawn(command, args, { cwd: extra.cwd ?? root, env: { ...env, ...(extra.env ?? {}) }, stdio: ["ignore", "pipe", "pipe"] });
...
try {
  ...
  const browserEnv = existsSync(installedBrowser) ? { ...env, PLAYWRIGHT_EXECUTABLE_PATH: installedBrowser } : env;
  if (!existsSync(installedBrowser)) {
  ...
  const test = spawn(localPlaywright, ["test", "e2e/youtube-studio-live.spec.ts", "--project=chromium"], { cwd: root, env: { ...browserEnv, PATH: `/home/linuxbrew/.linuxbrew/bin:${process.env.PATH ?? ""}` }, stdio: "inherit" });
