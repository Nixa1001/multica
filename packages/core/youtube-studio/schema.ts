...
// Version detail carries provenance in its nested object; the summary fields
// are intentionally not duplicated at the top level of this response.
export const StudioVersionSchema = z.object({ id:z.string(), artifact_id:z.string(), version_number:z.number(), content_kind:z.literal("markdown"), markdown:z.string(), sha256:z.string().regex(/^[0-9a-f]{64}$/), recorded_at:z.string(), provenance:z.object({source_result_id:z.string(),source_issue_id:z.string(),source_task_id:z.string(),producer:z.object({type:z.string(),id:z.string(),name:z.string()}).nullable()}) });
...
export const EMPTY_STUDIO_VERSION: StudioVersion = { id: "", artifact_id: "", version_number: 0, content_kind: "markdown", markdown: "", sha256: "0".repeat(64), recorded_at: "", provenance: { source_result_id: "", source_issue_id: "", source_task_id: "", producer: null } };
