...
func (h *Handler) YouTubeStudioVideo(w http.ResponseWriter, r *http.Request) {
	...
	materials := []any{}
	for rows.Next() {
		var bid, aid, iid, versionID, versionResultID, versionIssueID, versionTaskID, resultID pgtype.UUID
		var kind string
		...
		if e = rows.Scan(&bid, &aid, &kind, &iid, &number, &title, &status, &prefix, &hasIssue, &versionID, &version, &hash, &recorded, &versionResultID, &versionIssueID, &versionTaskID, &resultID, &resultRecorded, &attempts, &nextAttempt, &deadLettered, &consumed, &producerID, &producerName, &sourceState, &ingest); e != nil {
			writeErrorCode(w, 500, "studio_read_failed", "Studio read failed")
			return
		}
		var source any
		...
		var current any
		if versionID.Valid {
			current = map[string]any{"id": uuidToString(versionID), "version_number": version.Int32, "sha256": hash.String, "recorded_at": recorded.Time.UTC().Format(time.RFC3339Nano), "source_result_id": uuidToString(versionResultID), "source_issue_id": uuidToString(versionIssueID), "source_task_id": uuidToString(versionTaskID)}
		}
